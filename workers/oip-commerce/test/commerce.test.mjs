...
test("2045 object replacement preserves valid links and all existing access restrictions", async () => {
  const product = productForSku("OIP-TD-EPUB");
  assert.equal(product.r2Key, "epubs/oip-td.epub");
  assert.equal(product.priceCents, 1999);
  assert.equal(product.downloadFilename, "2045-ten-dark-fables.epub");
  const before = new Uint8Array([80, 75, 3, 4, 1, 2, 3]);
  const replacement = new Uint8Array([80, 75, 3, 4, 4, 5, 6, 7, 8]);
  const objects = new Map([[product.r2Key, before]]);
  const reads = [];
  const env = baseEnv({
    EPUB_BUCKET: {
      async get(key) {
    ...
    __testFetch: async (url) => {
      assert.match(url, /\/v2\/payments\/revision-refunded$/u);
      return Response.json({ payment: {
        id: "revision-refunded", order_id: "revision-refund-order",
        amount_money: { amount: 1999, currency: "USD" },
        refunded_money: { amount: 1999, currency: "USD" },
      } });
    },
  });
  const now = Math.floor(Date.now() / 1000);
  const cases = [
    ["valid", "A", {}],
    ["expired", "B", { expires_at: now - 1 }],
    ["exhausted", "C", { download_count: 5 }],
    ["revoked", "D", { status: "REVOKED" }],
    ["refunded", "E", { payment_id: "revision-refunded", order_id: "revision-refund-order" }],
  ];
  const urls = new Map();
  for (const [name, letter, overrides] of cases) {
    const token = letter.repeat(43);
    urls.set(name, `https://downloads.outsideinprint.org/download/${token}`);
    env.DB.fulfillments.set(name, {
      fulfillment_id: name, payment_id: `revision-${name}`, order_id: `revision-order-${name}`,
      sku: product.sku, status: "ACTIVE", token_sha256: await sha256Hex(token),
      expires_at: now + 3600, download_count: 0, max_downloads: 5, ...overrides,
    });
  }
  await processSquareEvent(env, {
    event_id: "revision-refund-event", type: "refund.updated",
    data: { object: { refund: { id: "revision-refund", payment_id: "revision-refunded", status: "COMPLETED" } } },
  }, now);
  assert.equal(env.DB.fulfillments.get("refunded").status, "REVOKED");
  const first = await handleRequest(new Request(urls.get("valid")), env);
  assert.equal(first.status, 200);
  assert.deepEqual(new Uint8Array(await first.arrayBuffer()), before);
  const validBefore = { ...env.DB.fulfillments.get("valid") };
  objects.set(product.r2Key, replacement);
  const second = await handleRequest(new Request(urls.get("valid")), env);
  assert.equal(second.status, 200);
  assert.deepEqual(new Uint8Array(await second.arrayBuffer()), replacement);
  assert.equal(second.headers.get("content-type"), "application/epub+zip");
  assert.equal(second.headers.get("content-disposition"), 'attachment; filename="2045-ten-dark-fables.epub"');
  assert.equal(second.headers.get("cache-control"), "private, no-store, max-age=0");
  assert.deepEqual(env.DB.fulfillments.get("valid"), { ...validBefore, download_count: 2 });
  for (const [name] of cases.slice(1)) {
    const recordBefore = { ...env.DB.fulfillments.get(name) };
    const denied = await handleRequest(new Request(urls.get(name)), env);
    assert.equal(denied.status, 404, name);
    assert.deepEqual(env.DB.fulfillments.get(name), recordBefore, `${name} must not consume or restore access`);
  }
  assert.deepEqual(reads, [product.r2Key, product.r2Key], "Denied links must not read the replacement object");
});
