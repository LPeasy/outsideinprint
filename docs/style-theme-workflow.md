...
At 768px and below, the homepage's `.masthead-controls` places the compact proof group between Paper Bob and the theme toggle above the unchanged wordmark. Both buttons remain 44px controls; the proof has no banner frame. Desktop and compact-page controls keep their existing positions. The homepage reader note preserves its complete copy and typography: `assets/js/home-reader-note.js` collapses only the suffix after the first two sentences through 720px. The native toggle exposes its state with `aria-expanded` and `aria-controls`; returning from desktop resets the mobile note. Without JavaScript the note stays complete and its toggle is hidden. Imprint and author links remain beneath the note, in equal-width mobile columns. Featured metadata items cannot split internally; decorative slashes disappear through 360px.
...
Within Featured Articles, the newest supporting card alone uses a contained 16:9 illustration above its copy and a `New!` tag; later supporting cards retain small square artwork beside their copy. The article link stays on the artwork, and a separate 44px zoom target shows a magnifier SVG on a pale disc. At 900px and below, stack the Featured Articles heading and introduction to preserve breathing room. Keep the image/dialog controls and narrow-screen artwork layouts readable without changing the Dolphin lead.
