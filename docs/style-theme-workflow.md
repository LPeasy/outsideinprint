...
At 768px and below, the homepage's `.masthead-controls` places Paper Bob and the theme toggle in a symmetric 44px row above the unchanged wordmark. Desktop and compact-page controls keep their existing positions. The homepage reader note preserves its complete copy and typography: `assets/js/home-reader-note.js` collapses only the suffix after the first two sentences through 720px. The native toggle exposes its state with `aria-expanded` and `aria-controls`; returning from desktop resets the mobile note. Without JavaScript the note stays complete and its toggle is hidden. Imprint and author links remain beneath the note, in equal-width mobile columns. Featured metadata items cannot split internally; decorative slashes disappear through 360px.

The homepage body follows one 70rem desktop measure in this order: stats, reader note, Featured Articles, library navigation, newsletter signup, and contribution. `home_reader_banner.html` owns the opening stats; `home_reader_newsletter.html` owns the separate signup region after the library controls while retaining the existing `home-reader-banner*` signup hooks. The `.home-v2-library.home-v2-next__links` navigation carries only Browse the library and Surprise me, using 44px controls and the shared wide page shell. The final `.home-v2-next` contribution region remains a single column, with no left border on `.home-v2-next__contribute`. Preserve these alignments and narrow-screen fallbacks when changing spacing or divider rules.
