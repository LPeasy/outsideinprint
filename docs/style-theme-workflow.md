...
Within Featured Articles, the newest supporting card alone uses a contained 16:9 illustration above its copy and a `New!` tag; later supporting cards retain small square artwork beside their copy. The article link stays on the artwork, and a separate 44px zoom target shows a magnifier SVG on a pale disc. The Dolphin lead uses the same zoom control inside the shared positioned artwork wrapper, while retaining its wide hero and eager, high-priority image loading. At 900px and below, stack the Featured Articles heading and introduction to preserve breathing room. Keep the image/dialog controls and narrow-screen artwork layouts readable.
