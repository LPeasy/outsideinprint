...
The 446-image focused-cleanup core cohort comprises the 337 R3 review images, 105 referenced legacy Medium PNGs, and four Syd-and-Oliver dialogue heroes. Routine published artwork may add approved `editorial_cartoon` and `essay_illustration` assets after that baseline; each new canonical asset must add exactly one resolver alias. The six Modern Bios photographs described below add an explicit `essay_photo` cohort without changing the 13-photo legacy baseline or counting photographs as illustrations. The focused cleanup keeps 316 compact, referenced Medium JPEG/JPG files under `static/images/medium`; those files remain ordinary static fallbacks because responsive conversion would increase deployed bytes. It removes 50 unreferenced Medium files and leaves no Medium PNG/GIF or Syd-and-Oliver source artwork under `static/`. Twelve managed originals remain intentionally unreferenced: three supplemental essay photos, two unique legacy PNG illustrations, the retired provisional RCP8.5 framing chart, and six retired abstract Modern Bios heroes. They use `usage_state: retained_unreferenced`; every other managed asset uses `usage_state: referenced`. Healthy assets require `review_state: approved` before release; the one corrupt quarantine uses the explicit rejected state described below.

Modern Bios uses six licensed or public-domain portraits at `essays/modern-bios/{charlie-kirk,dick-cheney,gene-hackman,george-foreman,ozzy-osbourne,pope-francis}/portrait`. Register them as `essay_photo` with the `photo` processing hint, one resolver alias each, subject-specific alternative text, and a descriptive caption. `data/image_credits.json` keys author, source URL, license, license URL, and changes by asset ID; the shared `images/credit.html` partial exposes linked credits on the article and expanded collection illustration. Their article `featured_image` references serve both surfaces, including existing magnifiers. Retain each replaced abstract hero's source, ID, alias, and hashes; change only its usage state to `retained_unreferenced`. The source contract explicitly allows those six retirements while keeping historical review and migration evidence unchanged. Their separate source/license and visual-review evidence lives in [Modern Bios portrait revisions](editorial-audits/image-revisions/modern-bios-portraits-20260919.md), not rewritten frozen baseline reports.
