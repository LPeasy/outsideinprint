...
- `/essays/` now owns a dedicated archive-first section shell through `layouts/essays/list.html` and the `essays-front*` namespace rather than riding on the generic section-landing template.
...
| Essays front | `/essays/` | Section `_index.md` plus `hugo.toml` permalinks; Hugo uses section-specific template lookup before `_default` | `layouts/essays/list.html` | `layouts/partials/discovery/page-list-item.html` | `.essays-front`, `.essays-front__masthead`, `.essays-front__stats`, `.essays-front__year-nav`, `.essays-front__year-jumps`, `.essays-front__year-link`, `.essays-front__archive`, `.essays-front__month`, `.essays-front__month-title`, `.essays-front__month-list`, shared `.item-kicker`, shared `.item-kicker--collection`, shared `.page-header`, shared `.page-intro`, shared `.page-shell--reading` | Route-owned archive shell layered on shared row partials and global page shells | Archive rows still reuse the shared `.item` structure by design, so row-level title/meta/summary behavior remains partly shared even though collection taxonomy moves into a route-specific kicker treatment. |
