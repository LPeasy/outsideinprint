...
- `/apps/` and `/apps/bucks-machine/` are public development-preview routes. They own a restrained product interface through the `apps-index`, `apps-card`, `apps-product`, `apps-status`, `apps-packet-preview`, `apps-workflow`, `apps-deliverables`, `apps-samples`, `apps-limitations`, and `apps-identity` namespaces while retaining the global OIP masthead, footer, theme, and page shells.
...
| Apps & Tools index | `/apps/` | Published `content/apps/_index.md` plus section-specific template lookup | `layouts/apps/list.html` | `layouts/partials/apps/product-data.html` | `.apps-index`, `.apps-index__header`, `.apps-index__intro`, `.apps-index__notice`, `.apps-index__grid`, `.apps-index__empty`, `.apps-card`, `.apps-card__copy`, `.apps-card__title`, `.apps-card__promise`, `.apps-card__audience`, `.apps-card__deliverables`, `.apps-card__link`, `.apps-card__preview`, `.apps-card__identity`, `.apps-status-row`, `.apps-status`, `.apps-status-note`, shared `.page-shell--wide` | Route-owned public development-preview catalog layered on the global OIP shell | The masthead and footer expose the published section. No homepage promotion or commercial action exists. |
...
- Confirm Apps & Tools preview ownership:
  ...
  - Build normally and verify `/apps/`, `/apps/bucks-machine/`, their navigation links, and their synthetic PDF/XLSX resources are present while commercial actions, forms, intake, pricing, and checkout remain absent.
