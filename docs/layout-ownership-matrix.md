...
- Collections now own their inner structure explicitly through `collection-grid`, `collection-card`, `collection-meta`, `collection-meta-*`, `collection-progress*`, `collection-items`, `collection-item-note`, `collection-item-state`, `collection-pill`, `collection-pill--visited`, `collection-start-here`, `collection-membership__*`, and the collection-detail `collection-room*` namespace.
...
- Confirm collection ownership:
  ...
  - Verify `collection-room`, `collection-room__header`, `collection-room__section`, `collection-room__section--overview`, `collection-room__section--entry`, `collection-room__section--progress`, `collection-room__section--items`, `collection-room__section--related`, `collection-meta-*`, `collection-progress`, `collection-progress__summary`, `collection-progress__actions`, `collection-progress__note`, `collection-items`, `collection-item-note`, `collection-item-state`, `collection-pill`, `collection-pill--visited`, `collection-start-here`, `collection-item--start-here`, and `collection-membership__*` all have explicit CSS owners.
