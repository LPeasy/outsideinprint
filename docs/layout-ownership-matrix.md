...
- `/duel/support/` and `/duel/privacy/` own a compact non-article information shell through `layouts/duel/single.html` and the `duel-info-route*` namespace, while reusing shared page shells, `piece-body` typography, and journey links.
...
| Duel information | `/duel/support/`, `/duel/privacy/` | Leaf bundles under `content/duel/` set `type: duel`; Hugo resolves to `layouts/duel/single.html` | `layouts/duel/single.html` | `layouts/partials/journey_links.html` | `.duel-info-route`, `.duel-info-route__header`, `.duel-info-route__body`, `.duel-info-route__nav`, `.duel-info-route__journey`, shared `.page-header`, shared `.page-intro`, shared `.piece-body`, shared `.page-shell--reading` | Route-owned product information shell layered on shared reading primitives | The pages intentionally avoid article bylines, citation rails, publication records, and newsletter modules. |
...
- Confirm Duel information route ownership:
  - Compare `layouts/duel/single.html` against `assets/css/main.css`.
  - Verify `duel-info-route`, `duel-info-route__header`, `duel-info-route__body`, `duel-info-route__nav`, and `duel-info-route__journey` have explicit CSS owners and the layout contains no article byline or publication record.
