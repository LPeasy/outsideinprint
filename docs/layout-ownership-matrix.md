...
- `/apps/` and `/apps/bucks-machine/` are unpublished, localhost-only draft routes. They own a restrained product interface through the `apps-index`, `apps-card`, `apps-product`, `apps-status`, `apps-packet-preview`, `apps-workflow`, `apps-deliverables`, `apps-samples`, `apps-limitations`, and `apps-identity` namespaces while retaining the global OIP masthead, footer, theme, and page shells.
...
| Homepage | `/` | Hugo home route resolves to `layouts/index.html` | `layouts/index.html` | `layouts/partials/home_front_page.html`, `layouts/partials/home_bookstore_spotlight.html`, `layouts/partials/home_imprint_statement.html`, `layouts/partials/home_selected_collections.html`, `layouts/partials/newsletter_signup.html`, `layouts/partials/discovery/page-list-item.html`, `layouts/partials/discovery/collection-card.html` | `.home-manifesto`, `.home-manifesto__inner`, `.home-manifesto__copy`, `.home-manifesto__line`, `.home-bookstore`, `.home-bookstore__header`, `.home-bookstore__grid`, `.home-bookstore__card`, `.home-almanack`, `.home-almanack-divider`, `.home-almanack__ledger`, `.home-front-page__stories`, `.home-front-page__lead`, `.home-front-page__secondary-kicker`, `.home-front-page__secondary-title a`, `.home-front-page__secondary-meta`, `.entry-threads`, `.entry-threads__grid`, `.home-browse__list`, `.newsletter-signup--home-ribbon`, shared `.item`, shared `.page-shell--wide`, shared `.page-shell--feature`, shared `.page-shell--grid` | Route composition is `route-owned`; most actual geometry is shared CSS and shared partial markup | `home-front-page__secondary`, `home-front-page__secondary-item`, `home-front-page__secondary-dek`, and `home-front-page__secondary-action` still rely on container flow and shared text utilities rather than dedicated selectors. |
...
| Apps & Tools index | `/apps/` | Draft `content/apps/_index.md` plus section-specific template lookup; omitted from normal production builds | `layouts/apps/list.html` | `layouts/partials/apps/product-data.html` | `.apps-index`, `.apps-index__header`, `.apps-index__intro`, `.apps-index__notice`, `.apps-index__grid`, `.apps-index__empty`, `.apps-card`, `.apps-card__copy`, `.apps-card__title`, `.apps-card__promise`, `.apps-card__audience`, `.apps-card__deliverables`, `.apps-card__link`, `.apps-card__preview`, `.apps-card__identity`, `.apps-status-row`, `.apps-status`, `.apps-status-note`, shared `.page-shell--wide` | Route-owned localhost draft catalog layered on the global OIP shell | The masthead and footer expose this route only when `hugo.IsServer` can resolve the draft section. No production navigation or homepage promotion exists. |
| Bucks Machine product | `/apps/bucks-machine/` | Draft leaf bundle at `content/apps/bucks-machine/index.md` plus the explicit Apps permalink family; omitted from normal production builds | `layouts/apps/single.html` | `layouts/partials/apps/product-data.html`, `layouts/partials/apps/actions.html`, `layouts/partials/apps/sample-downloads.html` | `.apps-product`, `.apps-product__hero`, `.apps-product__hero-copy`, `.apps-product__promise`, `.apps-product__audience`, `.apps-product__intake`, `.apps-status-row`, `.apps-status`, `.apps-status-note`, `.apps-packet-preview`, `.apps-workflow`, `.apps-deliverables`, `.apps-samples`, `.apps-limitations`, `.apps-identity`, shared `.page-shell--wide` | Route-owned localhost draft product page with local-only sample resources | PDF/XLSX links render only for a draft page on the local Hugo server when the data state is `local_draft`; `build.publishResources: false` prevents resource publication by the normal build. Commercial actions remain inert. |
...
- Confirm Apps & Tools draft ownership:
  - Compare `layouts/apps/list.html` and `layouts/apps/single.html` against `assets/css/main.css`.
  - Verify `apps-index`, `apps-card`, `apps-product`, `apps-status`, `apps-packet-preview`, `apps-workflow`, `apps-deliverables`, `apps-samples`, `apps-limitations`, and `apps-identity` all have explicit CSS owners.
  - Build normally and verify `/apps/`, `/apps/bucks-machine/`, their navigation links, and their sample PDF/XLSX resources are absent. Preview with `hugo server -D` and verify both routes are present.
