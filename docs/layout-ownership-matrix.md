...
| Collections index | `/collections/` | `content/collections/_index.md` plus section-specific template lookup | `layouts/collections/list.html` | `layouts/partials/discovery/collection-card.html` | `.section-front`, `.section-front--collections`, `.section-front__header`, `.page-header`, `.page-intro`, generic `.grid`, generic `.card`, `.collections-directory`, `.collections-directory__group`, `.collections-directory__group-title`, `.collections-directory__grid`, `.collection-grid`, `.collection-card`, `.collection-meta`, `.page-shell--wide`, `.page-shell--grid` | Route-owned unified card directory sitting on a shared top-zone shell plus shared grid/card primitives and collection-specific card modifiers | Card framing still inherits from shared `.card`, by design; the route no longer splits featured cards from a neutral row index and no longer exposes a top-level journey-links block. |
