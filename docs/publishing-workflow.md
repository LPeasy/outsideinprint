...
- Homepage cards with full-width artwork (the lead and newest supporting piece) place the title and reading-time metadata above the illustration. Compact supporting cards keep the title, reading time, and summary beside their small square artwork. All cards retain a single heading before the artwork in document order. Preserve these size-specific layouts for future selected pieces as well as the flagship.

- Homepage selection lives in `layouts/partials/home_v2_selected.html`. *The Dolphin Company* (`/essays/the-dolphin-company/`) is the flagship lead while it is eligible; otherwise the newest eligible reading piece fills the lead. The newest remaining eligible piece always occupies the first supporting position. Newness uses Hugo's `PublishDate` (explicit `publishDate`, falling back to `date`), with equal publication times ordered by title. The remaining editorial order is *What I Had* (`/syd-and-oliver/what-i-had/`), *Default Owner* (`/essays/default-owner/`), then *Reverse Origami* (`/essays/reverse-origami/`). Reverse Origami's source lives at `content/essays/musings/reverse-origami.md`. The Dolphin Company uses the homepage-only label `Case study`; its canonical `section_label` remains `Essay`. Dialogue and Musing labels follow existing content metadata, as do the other section labels.
- When a preferred route is already selected, unavailable, draft, future-dated, or expired, the selector skips it and fills remaining slots with the newest other eligible essays, affirmations, or dialogues, breaking equal dates by title, until the surface contains five unique pieces. The existing reading-kind resolver also includes Musings stored under essays. Canonical published URLs determine selection, including dialogues whose source paths differ from their public URLs. Draft, future-date, future-`publishDate`, and expiry guards apply even when a preview enables those pages; landing pages are not eligible.
- The first selected piece places metadata and its headline before the existing wide responsive `featured_image` hero, followed by its summary and article CTA. The newest supporting piece gets a publication date from `PublishDate`, a restrained freshness tag, and a larger, contained 16:9 illustration above its copy; the remaining supporting cards keep compact square artwork beside their copy. `home_feature_freshness.html` compares publication time against one build timestamp: `New!` applies from zero through less than 14 elapsed days, and `Latest` applies at 14 days and afterward. The existing daily rebuild updates the label on its next successful run; no client timer or additional schedule is needed. All supporting artwork links to its article, with a separate 44px zoom target: a magnifier SVG on a pale disc opens the native illustration dialog. At 768px and below, the supporting cards form one column; the compact cards retain a 6rem square artwork column. `home_featured_image_button.html`, `home_featured_image_dialog.html`, and `assets/js/home-featured-image.js` own the zoom enhancement: the native button starts hidden and JavaScript enables it, while a direct artwork link remains available without JavaScript. Supporting artwork uses the shared picture renderer with lazy loading; the newest card advertises its larger responsive slot, while compact cards use `sizes="120px"`. Clicking the expanded illustration or close button closes the dialog, and Escape provides keyboard closing. Use the existing article artwork and alt text; this surface does not require new illustrations or front-matter changes. A new publication takes the first supporting position on the next Hugo rebuild; changing only an older piece's modification time does not promote it. Editorial fit determines the remaining supporting order; audience figures do not sort this selection. Verify selection against the `publishDate` column from Hugo's published inventory, keeping five unique pieces when available. The archive sorts by article `date`, so its first entry is not a reliable publication-time oracle when explicit release times differ.
...
- Homepage composition places the unframed proof group (`home_reader_banner.html`) inside the homepage masthead controls, between Paper Bob and the theme toggle and above the Outside In Print wordmark. The body then presents the reader note, Featured Articles, library navigation, newsletter signup (`home_reader_newsletter.html`), and a centered `Become a contributor` button linking to `/contribute/`. The proof flexes up to 42rem: three cells above 400px, then two short metric rows beside a full-height Weekly Newsletter link at 400px and below. The audience uses the source-backed `10k+` compact alias there, while the full figure remains available to assistive technology. The newsletter link retains a 44px tap target. The former `Support Independent Media` introduction link and homepage subject sentence are absent; the About-page description remains unchanged. The whole Weekly Newsletter proof cell is a native link to the existing `home-reader-banner-title` heading; that heading has `tabindex="-1"` so keyboard navigation can continue into the form. The `Keep reading` navigation contains only Browse the library (`/library/`) and Surprise me (`/random/`); the homepage body has no Archive CTA, browse heading, or browse description. `/archive/` and the shared Read navigation retain archive discovery. The separate newsletter partial preserves the existing Buttondown form, IDs, privacy link, weekly heading, no-spam promise, and `homepage_reader_banner` analytics source slot. Below the form, it shows one brief from the current published Bob's Almanack edition: its date and issue number, lead writing, one number, and weekly virtue, with a link to the full issue. It uses the same published collection selection as the Almanack collection page and renders no extra signup form. The final `home-v2-next` region uses `aria-label="Become a contributor"` and contains only its 44px `home-v2-next__cta` link, with no article wrapper, heading, paragraph, border, or panel background. The `/contribute/` page remains the full contributor destination.
- The homepage lead uses `home_lead_summary.html`: prefer a nonempty, trimmed, plain-text `description`, otherwise use the existing discovery summary. Write descriptions as concrete invitations to read, not just alternate titles. Supporting cards and other discovery surfaces retain their existing summary precedence; do not change the shared resolver to tune only the homepage lead. GitHub's post-build homepage checks use production `hugo.toml` and verify description precedence, selection behavior, rendered destinations, and preserved composition.
...
The standard article opening keeps the title before a side image plate. `content/essays/musings/life-is-a-controlled-fall.md` is the opt-in image-led pilot: `image_led: true` places its resolved `featured_image` full width before the title, with the existing alt text, caption, and fullscreen control intact. Its desktop masthead is compact to leave room for the illustration. Leave other articles on the standard opening unless an image-led treatment is explicitly chosen.
...
- Long nonfiction in essays, reports, and working papers gets a closed native **In this article** contents menu when Hugo estimates at least eight minutes and the final rendered body has at least three major headings. Use the highest available heading level (H2, or H3 for legacy pieces), existing anchors, and existing heading text; exclude source/revision/author-note headings. Dialogues, fiction, Musings, Affirmations, source-free work, short pieces, and pages outside these reading sections do not get a menu. `article_contents: false` opts a specific piece out. `article/contents.html` reads the already-rendered body after legacy cleanup so removed headings never create dead links. Do not rewrite article copy or revision histories to add navigation.
- Bob's Almanack's collection archive offers **Get the weekly newsletter** near its introduction and mounts the shared signup once below its latest-issue preview. Keep the `almanack-collection-newsletter` target and `almanack_collection_newsletter` analytics source slot together; the empty archive fallback remains unchanged.
- The standalone 2045 sample ends with **Get the full e-book** and the canonical product price, linking to `/shop/2045/#bookstore-purchase`. **About the book** retains the overview destination and existing `bookstore_sample_continue` slot; the purchase-section link uses `bookstore_sample_buy`. Neither action starts a payment automatically.
- The contributor page uses the `contributor-pitch` shortcode for a native **Draft your pitch** mailto link with the existing subject and four prompt fields. The visible email address remains a fallback. URL-encode the draft body and CRLF line breaks; opening a draft must never send it automatically. Do not add subscriber consent or change contributor terms as part of this action.
- The Library opens with recent selections grouped by type, newest within each type. **Browse all [count] pieces** opens the complete catalog across types, newest first, with 24 results per page. `view=all` and `page` retain the view alongside existing search/filter/sort parameters; filtering resets page one, and Reset filters restores the grouped landing view. The version-1 JSON index keeps its search/filter fields and adds optional artwork data for illustrated results; image-exempt pieces stay text-only. JavaScript failure or absence preserves grouped selections and the Archive fallback.
- Standard reading-page endings show one collection-first continuation immediately after the body, before publication records and the existing newsletter form. Prefer an eligible topic, then a series, preserving front-matter preference within each kind. Show **More on [title]** or **More from [title]**, the collection description, and **Explore all [N] pieces →**. A valid destination must contain the current page and at least one other published piece, meet its configured public threshold or override, and have a published landing. Without a fit, show **Browse the library →**. Exclude draft, future, and expired pages even in previews. Header links keep their original front-matter order. Explicit custom and Studio endings remain unchanged. Article visit recording continues without visible progress counters; do not erase saved reading progress. Choose one strong collection when publishing, at most two; no per-article essay pairing is required. See [Collection continuation audit](collection-continuation-audit.md) for the September 2026 archive assignments and separate editorial follow-ups.
