...
- The first selected piece retains its existing wide responsive `featured_image` hero; supporting cards reuse their existing approved `featured_image` illustrations as square artwork on desktop and mobile. Supporting artwork links to the article at every viewport, with a separate zoom control for the native illustration dialog. At 768px and below, supporting cards form one column with a 6rem square artwork column beside each card's copy. `home_featured_image_button.html`, `home_featured_image_dialog.html`, and `assets/js/home-featured-image.js` own the zoom enhancement: the native button starts hidden and JavaScript enables it, while a direct artwork link remains available without JavaScript. Supporting artwork uses the shared picture renderer with lazy loading and `sizes="120px"`. Clicking the expanded illustration or close button closes the dialog, and Escape provides keyboard closing. Use the existing article artwork and alt text; this surface does not require new illustrations or front-matter changes. A new publication takes the first supporting position on the next Hugo rebuild; changing only an older piece's modification time does not promote it. Editorial fit determines the remaining supporting order; audience figures do not sort this selection. Verify selection against the `publishDate` column from Hugo's published inventory, keeping five unique pieces when available. The archive sorts by article `date`, so its first entry is not a reliable publication-time oracle when explicit release times differ.
