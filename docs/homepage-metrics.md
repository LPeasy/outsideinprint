...
`data/homepage_metrics.yaml` owns the homepage masthead figures and article audience figures. Templates consume its display labels and values; do not duplicate those figures in `hugo.toml` or template dictionaries. `reader_threshold` is the inclusive minimum value for a featured article badge. Article badges say **reads**; the masthead audience figure says **Readers**.

Each entry records `value`, `display_label`, `source`, `metric_definition`, `period`, `observed_at`, `evidence_ref`, `verification_status`, and `recorded_at`. Masthead entries also have `unit_label`; the audience entry has a `compact_label` for narrow screens. An entry's `recorded_at` says when its internal record was created, not when the underlying figure was observed. Unknown observation dates, evidence references, and periods remain `null` rather than receiving an invented date or source.
...
5. Build the site and confirm that only featured records with values at or above `reader_threshold` render badges and absent/sub-threshold entries have none. The Dolphin Company leads when eligible, followed by the newest remaining published reading page; What I Had, Default Owner, and Reverse Origami are the remaining curated picks. Duplicates are skipped and newest-first fallback retains five unique pieces when available. Metric refreshes never reorder the selection. No content front matter or canonical classification needs to change when metrics are refreshed.
