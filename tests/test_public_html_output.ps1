...
if ($targetPageHtml.ContainsKey('public/index.html')) {
  ...
  if ($supportParagraphs.Count -ne 0 -or $homeIndexHtml -match 'Support Independent Media') {
    $uxIssues.Add('public/index.html => expected no Support Independent Media introduction link')
  }
  if ($homeIndexHtml -match 'home-v2__subjects|Independent writing on history, economics, culture, and public life\.') {
    $uxIssues.Add('public/index.html => expected the former homepage subject sentence to stay absent')
  }
